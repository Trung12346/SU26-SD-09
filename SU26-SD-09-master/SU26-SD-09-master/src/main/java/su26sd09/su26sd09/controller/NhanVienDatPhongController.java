...
public class NhanVienDatPhongController {
    ...
    public String formThemDichVuBoSung(@RequestParam(required = false) Integer id,
        ...
        DatPhong dp = datPhongService.findById(id);
        if (dp == null) {
            model.addAttribute("error", "Không tìm thấy đơn đặt phòng #" + id);
            return "nhan-vien/them-dich-vu";
        }

        List<Chi_tiet_dich_vu> daSuDung = ctdvService.findByDatPhongId(id);
        BigDecimal tienDichVu = BigDecimal.ZERO;
        for (Chi_tiet_dich_vu ctdv : daSuDung) {
        ...
        model.addAttribute("daSuDung", daSuDung);
        model.addAttribute("tienDichVu", tienDichVu);
        model.addAttribute("dangO", "Da nhan phong".equals(dp.getTrangThai()));
    ...
    public String themDichVuBoSung(@RequestParam Integer id,
        ...
        if (!"Da nhan phong".equals(dp.getTrangThai())) {
            redirectAttributes.addFlashAttribute("error",
                    "Chỉ có thể thêm dịch vụ bổ sung khi đơn đang ở trạng thái 'Đã nhận phòng'.");
            return "redirect:/nhan-vien/dat-phong/them-dich-vu?id=" + id + "&page=" + page + "&size=" + size;
        }

        boolean coChon = false;
        if (dichVuIds != null) {
        ...
        if (!coChon) {
            redirectAttributes.addFlashAttribute("error", "Bạn chưa chọn dịch vụ nào.");
            return "redirect:/nhan-vien/dat-phong/them-dich-vu?id=" + id + "&page=" + page + "&size=" + size;
        }

        // Nếu đơn đã có hóa đơn (tạo trước đó), đồng bộ lại tổng tiền ngay.
        // Nếu chưa có hóa đơn, để hóa đơn được tạo gộp lúc trả phòng.
        if (hoaDonService.findByDatPhongId(id) != null) {
            hoaDonService.taoHoacCapNhatHoaDon(id);
        }

        redirectAttributes.addFlashAttribute("success", "Đã ghi nhận dịch vụ bổ sung cho đơn #" + id + ".");
        return "redirect:/nhan-vien/dat-phong/them-dich-vu?id=" + id + "&page=" + page + "&size=" + size;
    }
